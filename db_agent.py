#!/usr/bin/env python3
"""
DB Agent — 数据库与图片代理服务 (独立模式, 已不再默认使用)

⚠️ 注意: 本服务已于「图片服务合并」后内嵌进 ERP 服务, 接口由
   server/routers/db_agent_router.py 在 ERP 同一端口 (默认 8000) 上提供:
       GET  /ping  /file      POST /query  /execute
   正常部署无需再运行本文件, 也无需为 8765 单独开隧道。
   本文件仅作为「应急/独立部署」场景保留 (如 ERP 服务需要停机维护,
   又希望远程桌面上件助手仍可访问数据库时, 可临时单独启动)。
