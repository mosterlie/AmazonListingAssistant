    ...
    let html = `
      ...
      <div class="detail-section">
        ...
        <div class="detail-grid">
          ...
          <div style="grid-column:1/-1;"><div class="detail-field-label">产品分类</div><div class="detail-field-value" style="font-size:0.84rem; font-weight:600; word-break:break-all; color:#1d4ed8;">${p.category_name || '-'}</div></div>
...
function renderVariantDimensionCards(p) {
  ...
  // 以当前有效选项为准（避免编辑时删除的选项仍展示历史图片配置）；仅在无选项时回退到历史映射键
  const allKeys = options.length ? options : Object.keys(mapping);
  if (!allKeys.length) {
